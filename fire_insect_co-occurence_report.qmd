...
The distribution and connectivity of combustible fuels and fire weather jointly influence wildfire dynamics. Under extreme fire weather conditions the effects of fuels are less impactful and their effects do not emerge from models of wildfire behavior (Podur et al 2009). However, following defoliation we expect the impact of fuel to be greater(emerge, etc.). Therefore, we examined the drivers of burn severity within the subset of fires that had a history of defoliation (n = 256). We modeled burn severity as a function of host percentage, cumulative years of defoliation, time since defoliation, and several fire weather indices. Time since defoliation was modeled as a factor with 3 levels corresponding to three windows of opportunity. We included an interaction between cumulative years of defoliation and time since defoliation, as we predict defoliation recurring in successive years will have amplify the impacts of defoliation history depending on the time since defoliation. We fit a generalized least squares (GLS) model with a Gaussian spatial correlation structure, where correlation decreases with distance. Model fit was assessed by plotting residuals against fitted values (Appendix 1), and indicated a good fit with residuals centered around zero. To assess spatial autocorrelation, a variogram of the residuals was computed (Appendix 1) and plotted the spatial distribution of the model residuals @fig-spatial-residuals.
...
*Defoliation impacts on burn severity*
