...
post_climate_history = read.csv("/home/goldma34/fire_insect_co-occurence/data/outputs/on/on_history_final_era5_clim.csv")
...
In 2023, Canada experienced its most severe wildfire season on record, with 18.6 million hectares of forest burned. This, coupled with projected increases in the frequency and severity of outbreaks due to climate warming (Régnière et al 2012, Bouchard et al 2018, Weed et al 2013), raises concerns about the influence of these large-scale spatial legacies on wildfire behavior (Stocks 1987, Candau et al 2018, Fettig et al 2022). Adding to this concern is the climate-induced transition of black spruce from non-host to host species (Pureswaran et al 2019). Black spruce is the most widespread conifer in the eastern boreal forest. They are also considered to be the most flammable due to their resinous needles and their dense, low-lying branches that can easily catch fire and promote vertical spread of ground fires to high-severity canopy fires (Barrett et al 2010, Massey et al 2023).

Spruce budworm (SBW) defoliation significantly alters forest structure and the distribution and connectivity of combustible fuels across large areas. Vertical fuel continuity, which refers to the vertical distribution of fuels within a forest stand , increases with consecutive years of defoliation (Watt et al 2018). These vertical fuels, often called ladder fuels, facilitate the transition of surface fires to high-severity canopy fires (Van Wagner 1977). Additionally, surface fuel abundance increases by 145% in the years following outbreaks (Watt et al 2020).

Since the distribution of available fuel is a key driver of wildfire regimes, insect outbreaks are expected to have a strong bottom-up effect on wildfires (McCullough et al 1998). However, the cumulative effects of insect defoliation and subsequent natural disturbances, such as wildfire (James et al 2017), wind (Sturtevant et al 2014) and herbivory (Leroux et al 2021), often lead to non-linear responses that alter ecosystems in novel ways (Buma 2015). These non-linear responses contribute to the diversity of observed outcomes in past studies, and makes it challenging to disentangle how insect defoliation influences the emergent properties of subsequent disturbances.

Emergent properties of disturbance interactions are often categorized as additive, synergistic, or antagonistic (Sturtevant and Fortin 2021). It is plausible that SBW defoliation could alter wildfire behavior in ways that result in any of these interactions, depending on the severity of defoliation and its spatial or temporal legacies. However, few studies have empirically demonstrated these interactions.

Some research indicated that the spatial legacies of spruce budworm outbreaks increase fire risk (additive interaction) in Ontario (Stocks 1987, Fleming et al 2002a, James et al 2007) but decrease fire risk and severity (antagonistic interaction) in western North America (Lynch and Moorcroft 2008, Flower et al 2014). Additionally, the synergistic effects of defoliation on wildfire risk are limited to a specific time lag following defoliation. Outside of this "window of opportunity" of 8-10 years, the interaction may become antagonistic (James et al 2017, Fleming et al 2002a).

Simulations of spruce budworm outbreaks suggests the effects of fuel changes following defoliation will result in neutral or antagonistic interactions (James et al 2011, Vane et al 2017, Cohn et al 2014). Two possible scenarios contribute to the contrasting and uncertain effects of wildfire and spruce budworm interactions: (1) fires are inherently noisy systems, and (2) the rarity of insect-wildfire co-occurrence. Historical reconstructions suggests SBW outbreaks tend to occur when fire activity is low (Simard et al 2006).

The occurrence of wildfires is restricted by the abundance, structure, and connectivity of fuels (Miller and Urban 2000, Agee and Skinner 2005). However, in the eastern boreal forest, wildfire likelihood is primarily controlled by the top-down effects of fire weather. Under extreme fire weather conditions, the influence of fuels becomes negligible (Podur and Martell 2009). Therefore, it is possible that wildfire likelihood following defoliation is more influenced by weather than by forest fuels. James et al. (2017) supported this assertion by controlling for the variation explained by weather before modeling the effects of budworm legacies, thereby isolating the impact of SBW fuels on the likelihood of ignition. This highlights the challenge of disentangling clear cause-and-effect relationships given the significant role of weather and its interaction with fuels.

Fuel, rather than fire weather and climate, is the most critical factor influencing wildfire burn severity (Parks et al 2014a, 2018). Burn severity, which refers to the ecological impact of fire on vegetation and soil, is a key component of fire regimes. The level of burn severity determines how long and how well an ecosystem can recover to its pre-disturbed state (Bright et al 2019).

Interestingly, there is evidence that western spruce budworm (Choristoneura freeman) can reduce subsequent burn severity in the US Pacific Northwest (Meigs et al 2016). This reduction in severity is likely due to the forest recovery following budworm outbreaks, which often leads to the establishment and regeneration of nonhost species, such as broadleaf trees (Bouchard et al 2006). These species have higher moisture content and lower flammability (Parisien et al 2023).

Since burn severity is crucial for ecosystem recovery, it is essential to consider both recovery and burn severity together. The non-linear impacts from disturbance interactions may not be immediately apparent after a fire. Currently, no studies have explicitly investigated the influence of eastern spruce budworm legacies on wildfire burn severity in the eastern boreal forest. The cumulative effects of insect outbreaks and wildfires on post-fire recovery remain uncertain.
...
Fire severity is a measure of biomass and soil change used to gauge how intensely an ecosystem was impact by fire (Lentile et al 2013). A number of studies have posited that that spatial legacies of SBW defoliation will impact on wildfire effects (Cohn et al 2014; James et al. 2017). While there is evidence that supports insect-induced changes in vegetation structure prior to wildfire (Watt et al 2018, 2020), the causality between these changes and wildfire impacts such as changes in severity remain uncertain. This is because determining the causal impact of spruce budworm legacies on fire severity requires a control observation for comparison. However, most studies rely on historical defoliation and wildfire observational data. Identifying a control group is difficult as factors that influence fire outcomes include slope, vegetation type, or vegetation moisture. Therefore, establishing a control observation requires identifying statistically similar controls that burned under similar conditions. Fortunately, pre-processing methods such as propensity score matching are an established method for identifying such quasi-experimental controls in wildfire studies (Bustic, Woo x2 etc). Using pre-processed datasets, analysis such as linear regression can be used to predict average defoliation effect on wildfire severity.
...
-   Are there other factors that interact with defoliation to influence spatial patterns of burn severity in defoliated areas?
...
#| message: false
#| echo: false
#| label: Average treatment effect-full mod
...
best_model_info <- find_best_model(hist_gt90_1, methods, distances, links, m_orders, calipers, replace_list, use_mahvars_list)
...
best_model_info <- find_best_model(hist_gt90_2, methods, distances, links, m_orders, calipers, replace_list, use_mahvars_list)
...
best_model_info <- find_best_model(hist_gt90_3, methods, distances, links, m_orders, calipers, replace_list, use_mahvars_list)
...
#### Spatial patterns of burn severity in defoliated fires

Based on *a priori* expectations of east-west variations in vegetation type, composition, and climate, we examined the spatial patterns of burn severity for fires with a history of defoliation (n = 256). We modeled burn severity as a function of host percentage, cumulative years of defoliation, time since defoliation, and several fire weather indices. Time since defoliation was modeled as a factor with 3 levels corresponding to three windows of opportunity. We included an interaction between cumulative years of defoliation and time since defoliation, as we predict areas where defoliated occurred in successive years will have greater burn severity due to increase fuel loads. Therefore we would expect the interaction between cumulative years of defoliation and time since defoliation to reveal hotspots with a high degree of burn severity. We fit a generalized least squares (GLS) model with a Gaussian spatial correlation structure, where correlation decreases with distance. Model fit was assessed by plotting residuals against fitted values (Appendix 1), and indicated a good fit with residuals centered around zero. To assess spatial autocorrelation, a variogram of the residuals was computed (Appendix 1) and plotted the spatial distribution of the model residuals @fig-spatial-residuals.
...
Our findings highlight the important role of spatial legacies of spruce budworm defoliation on exacerbating the impacts of wildfire on post-fire biomass and soil change (burn severity) and recovery capacity and reveal variability in the impactfulness of spatial legacies based on the amount, and duration of defoliation and time since defoliation. Our resutls
