...
mixed.mod <- lmer(rbr_w_offset ~host_pct + Cumulative_Years_Defol*Time_Since_Defol + isi_90 + 
                    dc_90 + dmc_90 + ffmc_90 + bui_90+ fwi_90 + fire_area + x + y + (1| Fire_Year), data = defol_only)


t <- coef(summary(as(mixed.mod,"merModLmerTest")))
MuMIn::r.squaredGLMM(mixed.mod)

# Define the model formula
formula <- rbr_w_offset ~host_pct + Cumulative_Years_Defol*Time_Since_Defol + isi_90 + 
                    dc_90 + dmc_90 + ffmc_90 + bui_90+ fwi_90 + fire_area + x + y 

# Fit the model using nlme with spatial correlation
model_spatial <- lme(fixed = formula, 
                     random = ~ 1 | Fire_Year, 
...
# r2
r.squaredGLMM(model_spatial)

# extract residuals
defol_only$residuals <- residuals(model_spatial)
...
variogram_model <- variogram(residuals ~ 1, data = defol_only)

# Plot the variogram
plot(variogram_model, main = "Variogram of Residuals")
...
ggplot() +
  geom_sf(data = ontario, fill = "lightgrey", color = "black") +
  geom_point(data = df, aes(x = x, y = y, color = residuals)) +
